...
/**
 * Replace templates/parameters by values
 *
 * @param {Object} ast
 * @param {Object} data
 * @return {Object}
 */
exports.bind = function (ast, data) {
  return replaceParams(clone(ast), data);
};
...
function replaceParams(node, data) {
  Object.keys(node)
    ...
    .forEach(function (key) {
      ...
      if (!(typeof item === 'object' && item.type === 'param')) return replaceParams(item, data);

      if (data[item.value] === undefined) throw new Error('the template element not instantiated :' + item.value);
      node[key] = instantiateTpl(item, data[item.value]);
    });
